...
export default function BuilderPage() {
  ...
  // Camera mode (21 Sep 2026): the sidebar and its toggle go, the view is
  // the whole screen. isSidebarOpen is left alone so it comes back as it was.
  const cameraMode = useStore(s => s.cameraMode);
  const showSidebar = isSidebarOpen && !cameraMode;
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden relative">
        ...
        <div
          className={`shrink-0 bg-white/95 backdrop-blur-3xl shadow-[-4px_0_20px_rgba(0,0,0,0.08)] border-l border-black/5 z-10 flex flex-col order-1 md:order-2 transition-all duration-300 ease-in-out ${
            showSidebar ? 'h-[40vh] md:h-full md:w-[380px]' : 'h-0 md:h-full md:w-0 overflow-hidden'
          }`}
        >
