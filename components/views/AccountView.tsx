...
import { User, Shield, CreditCard, Clock, Calendar, ChevronRight, Zap, Sparkles, LogOut, Settings, History, Info, Image as ImageIcon, CheckCircle2, Trash2 } from 'lucide-react';
    ...
    return (
        <div className="h-full flex flex-col bg-background relative overflow-y-auto custom-scrollbar pt-20 pb-20">
            ...
            <div className="flex-1 px-8 md:px-16 relative z-10 w-full max-w-[1200px] mx-auto space-y-12">
                ...
                {/* Secondary Row: Brand Materials */}
                <div className="pt-12 animate-in fade-in slide-in-from-bottom-16 duration-1000 delay-600">
                    <div className="space-y-8">
                        <div className="flex items-baseline gap-4">
                            <h2 className="text-2xl font-bold text-accent tracking-tight">Brand Material Library</h2>
                            <div className="flex-1 h-px bg-border"></div>
                        </div>
                                    ...
                                    <div className="p-6 rounded-[2rem] bg-slate-50 border border-slate-100 space-y-6">
                                        <div className="flex flex-col md:flex-row gap-6">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-accent/60 pl-1">Swatch Image</label>
                                                <button 
                                                    onClick={() => document.getElementById(`upload-${category.id}`)?.click()}
                                                    className="w-20 h-20 shrink-0 rounded-2xl bg-white border border-border shadow-sm flex flex-col items-center justify-center text-secondary hover:text-accent hover:border-accent/30 hover:shadow-md transition-all relative overflow-hidden"
                                                >
                                                    {newItemForm[category.id].image ? (
                                                        <img src={newItemForm[category.id].image as string} className="absolute inset-0 w-full h-full object-cover" alt="preview" />
                                                    ) : (
                                                        <>
                                                            <ImageIcon size={20} className="mb-1" />
                                                            <span className="text-[8px] font-bold uppercase tracking-wider text-center px-2">Attach</span>
                                                        </>
                                                    )}
                                                </button>
                                                <input 
                                                    type="file" id={`upload-${category.id}`} className="hidden" accept="image/*"
                                                    onChange={(e) => {
                                                        ...
                                                        if (file) {
                                                            ...
                                                            reader.onload = (event) => {
                                                                ...
                                                                setNewItemForm(prev => ({
                                                                    ...prev,
                                                                    [category.id]: { ...prev[category.id], image: result }
                                                                }));
                                                            };
